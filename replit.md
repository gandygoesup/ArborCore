...
- **Configurable Estimate System**: Four-phase blueprint implementation:
  - **EstimateEngine Service**: Server-side pricing authority with preview/finalize pattern
  - **Field Registry**: Configurable estimate fields with `appliesTo` array for internal vs marketing contexts
  - **Pricing Profiles**: Default and custom pricing configurations (margins, tax, deposits)
  - **Pricing Rules**: Four effect types (flat, percentage, multiplier, perUnit) with field-based triggers
  - **Estimate Composer**: Dynamic field rendering with comparison options and side-by-side pricing
  - **Public Quote Widget**: Customer-facing quote UI using marketing fields with lead generation
